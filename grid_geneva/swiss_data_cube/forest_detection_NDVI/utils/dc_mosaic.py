...
def create_mean_mosaic(dataset_in, clean_mask=None, no_data=-9999, intermediate_product=None, **kwargs):
    ...
    ## RC added ==
    ndvi = (dataset_in_filtered.nir - dataset_in_filtered.red) / (dataset_in_filtered.nir + dataset_in_filtered.red)
    dataset_in_filtered['ndvi'] = ndvi 
    # does Nan/Nan= Nan ?? Otherwise we have a problem --> check the NDVI distribution
    ...
    return dataset_out.astype(kwargs.get('dtype', 'float64'))
...
def create_median_mosaic(dataset_in, clean_mask=None, no_data=-9999, intermediate_product=None, **kwargs):
    ...
    ndvi = (dataset_in_filtered.nir - dataset_in_filtered.red) / (dataset_in_filtered.nir + dataset_in_filtered.red)
    dataset_in_filtered['ndvi'] = ndvi
    #time_slices = range(len(dataset_in_filtered.time))
    #for timeslice in time_slices:
    #    dataset_slice = dataset_in_filtered.isel(time=timeslice)
    #    ndvi = (dataset_slice.nir - dataset_slice.red) / (dataset_slice.nir + dataset_slice.red)
    #    dataset_slice['ndvi'] = ndvi
    #    dataset_in_filtered = dataset_slice.copy(deep=True)
    ## ===========
